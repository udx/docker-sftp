...
function refreshKeys(updateKeysFn = updateKeys) {
  ...
  const configuredTimeoutMs = Number.parseInt(
    process.env.SSH_KEY_REFRESH_TIMEOUT_MS || "60000",
    10
  );
  const timeoutMs = configuredTimeoutMs > 0 ? configuredTimeoutMs : 60000;
  let completed = false;
  ...
  const timeout = setTimeout(() => {
    complete(new Error(`SSH key refresh timed out after ${timeoutMs}ms`));
  }, timeoutMs);
  timeout.unref?.();

  try {
    updateKeysFn({ accessToken }, complete);
  } catch (error) {
    complete(error);
  }
}
