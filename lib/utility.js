...
var firebaseApp = require("firebase-admin/app");
var firebaseDatabase = require("firebase-admin/database");
...
module.exports.getFirebase = function() {
    ...
    ].forEach(function requireFirebaseSetting(name) {
        if (!process.env[name]) {
            throw new Error(name + ' is required to initialize Firebase');
        }
    });
    ...
    return module._firebase = firebaseDatabase.getDatabase(app);
...
module.exports.getCollection = function(collectionName, sortField, done) {
    ...
    var _collection;
    try {
        _collection = exports.getFirebase().ref(collectionName);
    } catch (error) {
        var unavailableCollection = {
            orderByChild: function() { return unavailableCollection; },
            once: function() { return unavailableCollection; },
            on: function() { return unavailableCollection; },
...
module.exports.updateKeys = function updateKeys(options, callback) {
    ...
    if (require('fs').existsSync(updateKeysOptions.keysPath)) {
        ...
        return callback(
            new Error('authorized_keys directory missing: ' + updateKeysOptions.keysPath),
            { users: {} }
        );
    }
