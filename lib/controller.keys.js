            ...
            async.eachLimit(_.values(_applications), 3, function fetchCollaborators(data, callback) {
                ...
                axios(requestOptions)
                    .then(res => {
                        ...
                        const rateLimit = _.get(res, 'headers.x-ratelimit-remaining');
                        if (rateLimit === '0') {
                            const resetTime = new Date(_.get(res, 'headers.x-ratelimit-reset') * 1000);
                            console.error("GitHub rate limit exceeded. Resets at: %s", resetTime);
                            return callback(new Error('GitHub rate limit exceeded'));
                        }
                        console.log('GitHub API calls remaining: %s', rateLimit);

                        // get just the permissions, add users to application
                        const collaborators = ('object' === typeof body && body.length > 0 ? body : []);
                        console.log('Found %d collaborators for %s', collaborators.length, data._id);
                        
                        collaborators.forEach(function(thisUser) {
                            const userRole = thisUser.role_name || 'none';
                            const allowedRolesList = _.split(allowedRoles, ",");
                            const allowedProdRolesList = _.split(allowedRolesForProd, ",");
                            const isProdBranch = data.sshUser.includes('.' + productionBranch);

                            // Log role check
                            console.log('Checking user %s with role %s for %s (prod: %s)', 
                                thisUser.login, userRole, data.sshUser, isProdBranch);

                            // Check role permissions
                            if ((_.includes(allowedRolesList, userRole) && !isProdBranch) || 
                                (isProdBranch && _.includes(allowedProdRolesList, userRole))) {
                                ...
                                console.log('Added access for %s to %s', thisUser.login, data.sshUser);
                            } else {
                                console.log('Skipped %s: insufficient permissions (role: %s)', 
                                    thisUser.login, userRole);
                            }
                        });
                    ...
                    .catch(err => {
                        ...
                        if (err.response && err.response.status === 404) {
                            console.error("Repository not found: %s", data._id);
                        } else if (err.response && err.response.status === 403) {
                            console.error("Access forbidden for %s. Check token permissions.", data._id);
                        } else {
                            console.error("Error fetching collaborators for %s: %s", data._id, err.message);
                        }
                        callback();
                    });
            ...
            _.each(_applications[appID].containers, function(singleContainer) {
                ...
                if (writableKeys.length > 0) {
                    fs.writeFile(_container_path, writableKeys.join("\n"), { mode: 0o600 }, function(err) {
                        ...
                        console.log("Wrote SSH Key file for [%s] applications container [%s].", appID, _.get(singleContainer, 'podName'));
                    });
